...
public class DealsOutboxWsPublisher {
    ...
    public void pollAndPublish() {
        // Only handle WebSocket routes here; leave other routes (like 'requests') for their own workers
        ...
        for (DealsOutboxEntry e : entries) {
            try {
                ...
                // Non-WS routes are left untouched here (e.g., 'requests')
            } catch (Exception ex) {
                System.err.println("[OutboxWsPublisher] failed to publish entry id=" + e.getId() + " ex=" + ex);
            }
        }
    }
}
